...
class Network(nn.Module):
    def __init__(self, in_chn=4, wf=16, relu_slope=0.2):
        ...
        """
        self, in_chans=4, embed_dim=64, expan_att_chans=4,
        ...
        self.st_1 = nn.Sequential(
            ...
            *[TransformerBlock(embed_dim=16, squeezes=(4, 8), shuffle=16, expan_att_chans=4) for _ in range(8)]
        )
        
        self.st_2 = nn.Sequential(*[
            #NRB(2, in_size=wf*4, out_size=wf*4, relu_slope=relu_slope),
            *[TransformerBlock(embed_dim=64, squeezes=(4, 4), shuffle=8, expan_att_chans=4) for _ in range(8)]
            
        ])

        self.st_3 = nn.Sequential(*[
            #NRB(1, in_size=wf*16, out_size=wf*16, relu_slope=relu_slope),
            *[TransformerBlock(embed_dim=256, squeezes=(4, 2), shuffle=4, expan_att_chans=4) for _ in range(8)]
            
        ])
        
        self.st_4 =nn.Sequential(*[
                
            *[TransformerBlock(embed_dim=1024, squeezes=(4, 1), shuffle=2, expan_att_chans=4) for _ in range(8)]
